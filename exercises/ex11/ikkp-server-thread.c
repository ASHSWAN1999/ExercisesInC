...
{
    char segfault[0] = "Let's cause a segfault!"; //takes down the whole server :(
...
void* thread_functionality(void* sock) {
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        pthread_t thread;
        child_sock = malloc(1);
        *child_sock = connect_d;

        if( pthread_create( &thread, NULL,  thread_functionality, (void*) child_sock) < 0)
        {
